...
pub fn min_dnc_opt<T>(f: impl Fn(usize, usize) -> Option<T>, n: usize, m: usize) -> Vec<T>
where
    T: Clone + Default + Ord,
{
    assert!(m > 0);
    ...
    while let Some((imin, imax, jmin, jmax)) = task.pop() {
        ...
        let (opt_val, Reverse(opt_j)) = (jmin..=jmax)
            .filter_map(|j| f(mid, j).map(|val| (val, Reverse(j))))
            .min()
            .unwrap();
        ans[mid] = opt_val;
    ...
    T: std::ops::Add<Output = T> + Default + Clone + Ord,
{
    ...
    assert!(convex
        .windows(3)
        .all(|w| { w[2].clone() + w[0].clone() >= w[1].clone() + w[1].clone() }));
