pub mod lazy_segtree {
	...
	pub trait LazyMonoid: Copy + Debug {
		...
		/// Tries to apply the lazy into the value, and transforms the lazy if necessary.
		/// If unsuccessful (Segment Tree Beats), returns [`None`].
		fn unlazy(v: Self, size: usize, lz: Self::Lazy) -> Option<(Self, Self::Lazy)>;
	}
	...
	impl<T: LazyMonoid> LazySegtree<T> {
		...
		fn propagate(&mut self, i: usize, nl: usize, nr: usize) {
			...
			if let Some((v, lz)) = T::unlazy(self.arr[i], nr-nl+1, self.lazy[i]) {
				if i < self.n {
					self.lazy[i*2] = T::op_lazy(self.lazy[i*2], lz);
					self.lazy[i*2+1] = T::op_lazy(self.lazy[i*2+1], lz);
				}
				self.arr[i] = v;
